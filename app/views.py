...
def mobile(request, data=None):
  if data == None:
    ...
    mobiles = Product.objects.filter(category='M').filter(discounted_price__gt=4000)
...
def bottomwear(request, data=None):
	...
	if data==None :
			bottomwears = Product.objects.filter(category='BW')
	elif data == 'PUMA' or data == 'UM':
			bottomwears = Product.objects.filter(category='BW').filter(brand=data)
	elif data == 'below':
			bottomwears = Product.objects.filter(category='BW').filter(discounted_price__lt=1050)
	elif data == 'above':
